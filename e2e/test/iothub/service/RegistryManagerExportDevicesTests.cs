    ...
    public class RegistryManagerExportDevicesTests : E2EMsTestBase
    {
        ...
        public async Task RegistryManager_ExportDevices(StorageAuthenticationType storageAuthenticationType, bool isUserAssignedMsi)
        {
            ...
            try
            {
                ...
                JobProperties exportJobResponse = await RunExportJobAsync(
                ...
                exportJobResponse = await WaitForJobToCompleteAsync(registryManager, exportJobResponse).ConfigureAwait(false);
        ...
        private async Task<JobProperties> RunExportJobAsync(StorageAuthenticationType storageAuthenticationType, bool isUserAssignedMsi, string devicesFileName, string configsFileName, RegistryManager registryManager, Uri containerUri)
        {
        ...
        private async Task<JobProperties> WaitForJobToCompleteAsync(RegistryManager registryManager, JobProperties exportJobResponse)
        {
            ...
            Device device)
        {
            ...
            foreach (string serializedDevice in serializedDevices)
            {
                ...
                if (foundEdge2InExport && foundDeviceInExport)
                {
                    // we're done
                    break;
                }
                ...
                if (StringComparer.Ordinal.Equals(exportedDevice.Id, edge1.Id) && exportedDevice.Capabilities.IotEdge)
                {
                    ...
                    exportedDevice.ParentScopes.Should().BeEmpty("Top level edges shouldn't have a parent scope");
                    continue;
                }
        ...
        private async Task CleanUpDevicesAsync(string edgeId1, string edgeId2, string deviceId, string configurationId, RegistryManager registryManager)
        {
            try
            {
